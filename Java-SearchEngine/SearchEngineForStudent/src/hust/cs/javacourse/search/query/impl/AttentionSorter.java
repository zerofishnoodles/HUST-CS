...
public class AttentionSorter implements Sort {
    public AttentionSorter() {
    }
    ...
    public double score(AbstractHit hit) {
        ...
        for (AbstractTerm term : hit.getTermPostingMapping().keySet()){
            posting = hit.getTermPostingMapping().get(term);
            assert posting.getPositions().size() == posting.getFreq();
            for(int i=0;i<posting.getPositions().size();i++){
                score += (1.0/(posting.getPositions().get(i)+1))*10;
            }
        }
        return score;
    }
